    ...
    <Card className="bg-card border-border">
      <CardHeader>
        <div className="flex items-center justify-between">
          <CardTitle className="text-foreground text-base">Expérience professionnelle</CardTitle>
          {!showForm && !editingId && (
            <Button
              size="sm"
              variant="outline"
            ...
            {editingId === exp.id ? (
              ...
              <div className="flex items-start justify-between">
                <div>
                  ...
                  {exp.location && <p className="text-muted-foreground text-xs mt-0.5">{exp.location}</p>}
...
function ExperienceFormFields({
  ...
  return (
    <form action={onSubmit} className="space-y-3 p-3 rounded-lg border border-border bg-secondary/30">
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          ...
          <Input name="title" required defaultValue={defaultValues?.title || ""}
            ...
            className="bg-secondary border-border text-foreground text-sm placeholder:text-muted-foreground focus:border-lime-400/50" />
        </div>
        <div className="space-y-1">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="space-y-1">
          ...
          <Input name="startDate" type="date" defaultValue={formatDateForInput(defaultValues?.startDate)}
            className="bg-secondary border-border text-foreground text-sm focus:border-lime-400/50" />
        </div>
        <div className="space-y-1">
          ...
          <Input name="endDate" type="date" disabled={isCurrent}
            ...
            className="bg-secondary border-border text-foreground text-sm focus:border-lime-400/50 disabled:opacity-50" />
        </div>
      </div>

      <div className="flex items-center gap-3">
        <button type="button" onClick={() => setIsCurrent(!isCurrent)}
          className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${isCurrent ? "bg-lime-400" : "bg-secondary"}`}>
        ...
        <Label className="text-muted-foreground text-xs">Poste actuel</Label>
      </div>

      <div className="flex gap-2 pt-1">
        <Button type="submit" size="sm" disabled={isPending}
          className="bg-lime-400 text-black hover:bg-lime-300 font-semibold text-xs">
