    ...
    <div className="space-y-6 max-w-3xl">
      ...
      <div className="rounded-lg border border-border bg-card p-6">
        <div className="flex items-center gap-2 mb-3">
          <span className="text-foreground font-medium text-sm">{ticket.user.name || ticket.user.email}</span>
        ...
        <p className="text-foreground/80 text-sm whitespace-pre-wrap">{ticket.message}</p>
      </div>
      ...
      {ticket.replies.map((reply) => {
        ...
        return (
          ...
          >
            <div className="flex items-center gap-2 mb-3">
              <span className="text-foreground font-medium text-sm">{reply.user.name || reply.user.email}</span>
              {isAdmin && (
            ...
            <p className="text-foreground/80 text-sm whitespace-pre-wrap">{reply.message}</p>
          </div>
        )
      })}
      ...
      {ticket.status === "CLOSED" ? (
        <Card className="bg-card border-border">
          <CardContent className="py-8 text-center">
            <p className="text-muted-foreground mb-4">Ce ticket est fermé</p>
