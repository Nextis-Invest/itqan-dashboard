...
export function GigActions({ gigId, status }: { gigId: string; status: string }) {
  ...
  return (
    <div className="flex gap-1">
      ...
      {status === "ACTIVE" && (
        <Button size="sm" variant="ghost" disabled={loading} className="h-7 w-7 p-0 text-neutral-400 hover:text-yellow-400"
      ...
      {(status === "PAUSED" || status === "DRAFT") && (
        <Button size="sm" variant="ghost" disabled={loading} className="h-7 w-7 p-0 text-neutral-400 hover:text-lime-400"
          onClick={async () => { setLoading(true); await updateGigStatus(gigId, "ACTIVE"); setLoading(false) }}>
          {loading ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <Play className="h-3.5 w-3.5" />}
        </Button>
      )}
      {status !== "DELETED" && (
        <Button size="sm" variant="ghost" disabled={loading} className="h-7 w-7 p-0 text-neutral-400 hover:text-red-400"
